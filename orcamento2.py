...
def processar_entrada(entrada):
    ...
    i = 0
    while i < len(linhas):
        # Regex para capturar códigos de comprimento variável no início da linha
        match = re.match(r'^(\d+)\s+(\w+)\s+(\w+)\s+(.+)', linhas[i])
        if match:
            codigo, substancia, laboratorio, apresentacao = match.groups()
...
def main():
    # Exemplo de entrada com código de comprimento variável
    entrada = """12121 Metoprolol NQG 10'S 
1
34,50 10,00
30,55
30,55

14515 Sumatripitana MEG 20'S
2
20,00 10,00
18,00
32,00"""
